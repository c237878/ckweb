...
网格列数只有三档，按视口切换，不再用 auto-fill 猜列：

| 视口 | `.grid`（影片/漫画） | `.grid--rows`（演员/系列行卡） |
| --- | --- | --- |
| < 900px | 2 列 | 1 列 |
| ≥ 900px | 4 列 | 2 列 |
| ≥ 1400px | 6 列 | 3 列 |

卡片内的胶囊**统一一行、放不下自动换行**（`.pills` 是 flex + wrap），
不再按宽度分多行隐藏。原因：多行分档会让空间利用率变差，一排排同形状的胶囊也显乱。

**可点与不可点靠 hover 区分，不靠形状**：`.tag` 外观统一，
只有 `a.tag` / `button.tag` 有 hover（全局一条规则，描边取 `currentColor`），
纯 `span.tag` 拿不到。所以新增胶囊时选对元素标签就行，别自己写 hover。

唯一保留的宽度门槛是演员胶囊（`@container (min-width: 240px)`）：
一部片可能挂十几个演员，全渲染会让卡片高度失控。

`.video-card` / `.comic-card` 都设了 `container-type: inline-size`，
所以这个门槛看的是卡片实际宽度而不是视口。
